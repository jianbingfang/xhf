...
<div class="row-fluid">
    ...
    <article class="m-widget multiple">
        ...
        <div id="jlShizhong-infoSearch" class="content content-inner">
            <form name="jlShizhong-infoForm" method="post"
                  action="jlShizhong-info-list.do?&fszleix=<%= request.getParameter("fszleix")%>&style=${divStyle}"
                  class="form-inline">
                <label for="jlShizhong-info_name"><spring:message
                        code='jlShizhong-info.jlShizhong-info.list.search.name' text='名称'/>:</label>
        ...
        <div class="pull-left">
            <region:region-permission permission="jlShizhong-info:create">
                <button class="btn btn-small a-insert"
                        onclick="location.href='jlShizhong-info-input.do?fszleix=<%= request.getParameter("fszleix")%>&style=${divStyle}'">
                    新建
                </button>
    ...
    <article class="m-widget style upload">
        <div class="content">
            <form id="upload-infoGridForm" name="jlShizhong-infoGridForm"
                  method='post'
                  action="jlShizhong-info-remove.do?fszleix=<%= request.getParameter("fszleix")%>&style=${divStyle}"
                  class="m-form-blank">
                ...
                <div class="control-group">
                    ...
                    <div class="controls">
                        <select class="form-control" id="jlShizhong-info_fastus"
                            ...
                            <option value="是"
                                    <c:if test="${page.result[0].fastus=='是'}"> selected="selected" </c:if>>是
                            </option>
                            <option value="否"
                                    <c:if test="${page.result[0].fastus=='否'}"> selected="selected" </c:if>>否
                            </option>
                        </select>
                    </div>
                </div>
                ...
                <table id="uploadMonth-infoGrid" class="m-table table-hover table-bordered">
                    ...
                    <c:forEach items="${page.result}" var="item">
                        <tr>
                            ...
                            <td>${item.fuploaddate}</td>
                            <td>
                                <a href="jlShizhong-info-input.do?id=${item.fid}&fszleix=<%= request.getParameter("fszleix")%>&style=${divStyle}"
                                   class="a-update"><spring:message code="core.list.edit"
                                                                    text="编辑"/></a></td>
                        </tr>
                    </c:forEach>
    ...
    <article class=" style async">
        <div class="content">
            <form id="async-infoGridForm" name="jlShizhong-infoGridForm" method='post'
                  action="jlShizhong-info-remove.do?fszleix=<%= request.getParameter("fszleix")%>&style=${divStyle}"
                  class="m-form-blank">
