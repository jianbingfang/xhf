        ...
        <article class="m-widget">
            ...
            <div class="content content-inner">
                <form id="pjXm-infoForm" method="post"
                        ...
                        <div class="span5">
                            ...
                            <div class="controls">
                                <select class="form-control" id="pjXm-info_fxmtype"
                                    ...
                                    <option value="房屋建筑工程"
                                            <c:if test="${model.fxmtype=='房屋建筑工程'}"> selected="selected" </c:if>>房屋建筑工程
                                    </option>
                                    <option value="市政公用工程"
                                            <c:if test="${model.fxmtype=='市政公用工程'}"> selected="selected" </c:if>>市政公用工程
                                    </option>
                                    <option value="电力工程"
                                            <c:if test="${model.fxmtype=='电力工程'}"> selected="selected" </c:if>>电力工程
                                    </option>
                                    <option value="冶炼工程"
                                            <c:if test="${model.fxmtype=='冶炼工程'}"> selected="selected" </c:if>>冶炼工程
                                    </option>
                                    <option value="公路工程"
                                            <c:if test="${model.fxmtype=='公路工程'}"> selected="selected" </c:if>>公路工程
                                    </option>
                                    <option value="矿山工程"
                                            <c:if test="${model.fxmtype=='矿山工程'}"> selected="selected" </c:if>>矿山工程
                                    </option>
                                    <option value="化工石油工程"
                                            <c:if test="${model.fxmtype=='化工石油工程'}"> selected="selected" </c:if>>化工石油工程
                                    </option>
                                    <option value="水利水电工程"
                                            <c:if test="${model.fxmtype=='水利水电工程'}"> selected="selected" </c:if>>水利水电工程
                                    </option>
                                    <option value="农林工程"
                                            <c:if test="${model.fxmtype=='农林工程'}"> selected="selected" </c:if>>农林工程
                                    </option>
                                    <option value="铁路工程"
                                            <c:if test="${model.fxmtype=='铁路工程'}"> selected="selected" </c:if>>铁路工程
                                    </option>
                                    <option value="港口与航道工程"
                                            <c:if test="${model.fxmtype=='港口与航道工程'}"> selected="selected" </c:if>>
                                        港口与航道工程
                                    </option>
                                    <option value="航天航空工程"
                                            <c:if test="${model.fxmtype=='航天航空工程'}"> selected="selected" </c:if>>航天航空工程
                                    </option>
                                    <option value="通信工程"
                                            <c:if test="${model.fxmtype=='通信工程'}"> selected="selected" </c:if>>通信工程
                                    </option>
                                    <option value="机电安装工程"
                                            <c:if test="${model.fxmtype=='机电安装工程'}"> selected="selected" </c:if>>机电安装工程
                                    </option>
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="span5">
                            ...
                            <div class="controls">
                                <select class="form-control" id="pjXm-info_fxmstatus"
                                    ...
                                    <option value="在建"
                                            <c:if test="${model.fxmstatus=='在建'}"> selected="selected" </c:if>>在建
                                    </option>
                                    <option value="完工"
                                            <c:if test="${model.fxmstatus=='完工'}"> selected="selected" </c:if>>完工
                                    </option>
                                    <option value="竣工"
                                            <c:if test="${model.fxmstatus=='竣工'}"> selected="selected" </c:if>>竣工
                                    </option>
                                    <option value="停工"
                                            <c:if test="${model.fxmstatus=='停工'}"> selected="selected" </c:if>>停工
                                    </option>
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="controls">
                            <div class="input-append htPicker">
                                <input id="xzChufa-info_fhtid" type="hidden" name="fhtid"
                                    ...
                                    id="xmBxRecord-info_xmmc" type="text" value="${htMap[model.fhtid].fhtname}"
                                    readonly="readonly" <%--onclick="window.open('../gcgl/gcgl-commHt-info-input.do?id=${model.fhtid}')"--%>
                                    class=" required" style="width: 175px;" value=""> <span
                                    class="add-on" style="padding-top: 2px; padding-bottom: 2px;"><i
                    ...
                    <div class="control-group">
                        <label class="control-label" for="pjXm-info_fxmno"><spring:message
                                code="pjXm-info.pjXm-info.input.fxmno" text="编号"/></label>

                        <div class="controls">
                            <div class="input">
                                <input id="xmBxRecord-info_fxmno" type="text" name="fxmno"
                                       value="${model.fxmno}" readonly="readonly" class=""
                                       style="width: 175px;">
                            </div>
                        </div>
                    </div>
