...
<form id="pjRy-infoForm" method="post" action="pjRy-info-addsave.do" class="form-horizontal">
  <c:if test="${model != null}">
	  <input id="pjRy-info_id" type="hidden" name="fid" value="${model.fid}">
  </c:if>
			    	<div class="control-group">
				    <label class="control-label" for="pjRy-info_fxmid"><spring:message code="pjRy-info.pjRy-info.input.fxmid" text="项目"/></label>
					<div class="controls">
			    	...
			    	<div class="control-group">
				    <label class="control-label" for="pjRy-info_fgangwei"><spring:message code="pjRy-info.pjRy-info.input.fgangwei" text="岗位"/></label>
					<div class="controls">
							<select class="form-control" id="pjXm-info_fgangwei"
										name="fgangwei" value="${model.fgangwei}" class=" ">
									<option value="总监"
										<c:if test="${model.fgangwei=='总监'}"> selected="selected"  </c:if>>总监</option>
									<option value="总监代表"
										<c:if test="${model.fgangwei=='总监代表'}"> selected="selected"  </c:if>>总监代表</option>
									<option value="专监"
										<c:if test="${model.fgangwei=='专监'}"> selected="selected"  </c:if>>专监</option>
									<option value="监理员"
										<c:if test="${model.fgangwei=='监理员'}"> selected="selected"  </c:if>>监理员</option>
								</select>
				    </div>
					</div>
					<div class="control-group">
			    	...
			    	<div class="control-group">
				    <label class="control-label" for="pjRy-info_fstartdate"><spring:message code="pjRy-info.pjRy-info.input.fstartdate" text="进入项目时间"/></label>
					<div class="controls">
				  ...
				  <div class="control-group">
				    <label class="control-label" for="pjRy-info_fmemo"><spring:message code="pjRy-info.pjRy-info.input.fmemo" text="备注"/></label>
					<div class="controls">
					    	<input id="pjRy-info_fmemo" type="text" name="fmemo" value="${model.fmemo}" size="40" class="text "   style="background-color:white;cursor:default; width: 175px;">
				    </div>
				  </div>
