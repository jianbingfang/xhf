				...
				<div id="cwKaiPiao-infoSearch" class="content content-inner">
					<form name="cwKaiPiao-infoForm" method="post"
						...
						<button class="btn btn-small a-search"
							onclick="document.cwKaiPiao-infoForm.submit()">查询</button>
						&nbsp;
					</form>
				</div>
			</article>
				...
				<div class="pull-left">
					<region:region-permission permission="cwKaiPiao-info:create">
						<button class="btn btn-small a-insert"
							onclick="location.href='cwKaiPiao-info-input.do'">新建</button>
							...
							<tbody>
								<c:forEach items="${page.result}" var="item">
									<tr>
										...
										<td>${item.fdzdate}</td>
										<td><fmt:formatNumber type="number" value="${item.fdebit}" maxFractionDigits="2" /></td>
										...
										<%-- <td>${item.fmemo}</td> --%>
									  <td>
         					 <a href="cwKaiPiao-info-input.do?id=${item.fid}" class="a-update"><spring:message code="core.list.edit" text="编辑"/></a>
       						 </td>
									</tr>
								</c:forEach>
							</tbody>
