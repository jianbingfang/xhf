    ...
    <script type="text/javascript">
        ...
        function saveSelected() {
            ...
            $.each($('#selectResult tr'), function (index, tr) {
                var select = {
                    fid: $(tr).find('[name="selectedItem"]').val(),
                    zjid: $(tr).find('[filed="zjid"]').html(),
                    ftype: $(tr).find('[filed="zjtype"]').html(),
                    fupdatetime: $(tr).find('[filed="updatetime"]').html()
                };
                zjArr.push(select);
            });
...
<div class="row-fluid">
    ...
    <section id="m-main" class="span10">
                ...
                <div class="modal-content">
                    ...
                    <div class="container" style="width:900px;height: 500px;overflow: scroll;">
                        <div style="margin-bottom:5px;">
                            ...
                            <button type="button" class="btn  btn-small btn-primary" id="confirmBtn"
                                    onclick="selectItem()">确定
                            </button>
                            &nbsp;
                            ...
                            <div class="pull-right" style="widows: 400px;">
                                ...
                                <div class=" pull-right">
                                    每页显示<input id="selectPage" value="20" style="width: 40px;"> 条&nbsp;当前第<span
                                        id="pageNo">1</span>页
                        ...
                        <div id="xzZjOrderList-infoSearch2" class="content content-inner searchDiv" style="">
                            <form name="hrZj-infoForm" method="post"
                                ...
                                <option value="高级"
                                        <c:if test="${param.filter_EQS_fzhuanye=='高级'}"> selected="selected" </c:if>
                                        value="高级">高级
                                </option>
                                <option value="中级"
                                        <c:if test="${param.filter_EQS_fzhuanye=='中级'}"> selected="selected" </c:if>
                                        value="中级">中级
                                </option>
                                <option value="初级"
                                        <c:if test="${param.filter_EQS_fzhuanye=='初级'}"> selected="selected" </c:if>
                                        value="初级">初级
                                </option>
                        ...
                        <article class="m-widget">
                            <header class="header">
                                <h4 class="title"><spring:message
                                        code="xzZjOrderList-info.xzZjOrderList-info.list.title" text="列表"/></h4>
                            </header>
                            <div class="content" style="width:899px;height:700px; overflow:auto;">
                                <form id="xzZjOrderList-searchGridForm" name="xzZjOrderList-searchGridForm"
                                      method='post' class="m-form-blank">
                                    <table id="xzZjOrderList-searchGrid" class="m-table table-hover table-bordered">
                                        ...
                                        <tr>
                                            <th width="10" class="m-table-check"><input type="checkbox" name="checkAll"
                                                                                        onchange="toggleSelectedItems(this.checked,1)">
                                            </th>
                        ...
                        <div class="span5">
                            ...
                            <div class="controls">
                                <select id="xzZjOrder-info_fstatus" type="text" name="fstatus">
                                    <option value="已申请"
                                            <c:if test="${model.fstatus=='已申请'}"> selected="selected" </c:if>>已申请
                                    </option>
                                    <option value="已领取"
                                            <c:if test="${model.fstatus=='已领取'}"> selected="selected" </c:if>>已领取
                                    </option>
                                    <option value="已取消"
                                            <c:if test="${model.fstatus=='已取消'}"> selected="selected" </c:if>>已取消
                                    </option>
                                    <option value="已超时"
                                            <c:if test="${model.fstatus=='已超时'}"> selected="selected" </c:if>>已超时
                                    </option>
                                    <option value="已归还"
                                            <c:if test="${model.fstatus=='已归还'}"> selected="selected" </c:if>>已归还
                                    </option>
                                </select>
                            </div>
                        </div>
        ...
        <article class="m-widget">
            ...
            <div class="content">
                <form id="xzZjOrderList-infoGridForm" name="xzZjOrderList-infoGridForm" method='post'
                      action="xzZjOrderList-info-remove.do" class="m-form-blank">
