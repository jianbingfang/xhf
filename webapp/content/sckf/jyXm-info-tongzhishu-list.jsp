...
<script type="text/javascript">
	...
	$(function() {
		...
		createxmPicker({
			...
			redUrl : '${scopePrefix}/sckf/jyXm-info-tongzhishu.do'
		});
	});
</script>
</head>
		...
		<section id="m-main" class="span10">
			...
			<article class="m-widget">
				...
				<div id="jyXm-infoSearch" class="content content-inner">
					...
					<form name="jyXm-infoForm" method="post" action="jyXm-info-tongzhishu-list.do"
						class="form-inline">
						...
						<table id="jyXm-infoGrid" class="m-table table-hover table-bordered">
							<thead>
								<tr>
									...
									<th class="sorting" name="fzbdldw">招标代理机构</th>
									<th class="sorting" name="fzbtzslqr">中标通知书领取人</th>
									<th class="sorting" name="fzbtzsdate">领取时间</th>
							...
							<tbody>
								<c:forEach items="${page.result}" var="item">
									<tr>
										...
										<td>${item.fname}</td>
										<td>${item.fzbdldw}</td>
										<td>${item.fzbtzslqr}</td>
										<%-- <td>${item.fscale}</td> --%>
										<td>${item.fzbtzsdate}</td>
										<td>${item.fjieshouren}</td>
										<td>${item.fzbmoney}</td>
										<td><a href="jyXm-info-input.do?xmid=${item.fid}&state=tongzhishu"
