...
<script type="text/javascript">
	...
	$(function() {
		...
		createxmPicker({
			...
			redUrl : '${scopePrefix}/sckf/jyXm-info-toubiao.do'
		});
	});
</script>
</head>
		...
		<section id="m-main" class="span10">
			...
			<article class="m-widget">
				...
				<div id="jyXm-infoSearch" class="content content-inner">
					...
					<form name="jyXm-infoForm" method="post" action="jyXm-info-toubiao-list.do"
						class="form-inline">
						...
						<table id="jyXm-infoGrid" class="m-table table-hover table-bordered">
							<thead>
								<tr>
									...
									<th class="sorting" name="fbszzren">标书制作人</th>
									<th class="sorting" name="fpsren">审核人</th>
									<th class="sorting" name="ffuzeren">主办人</th>
									<th class="sorting" name="fzjdc">总监是否到场</th>
									<th class="sorting" name="ftbtjf">无法满足的条件</th>
									<th class="sorting" name="ftype">项目类别</th>
									<th width="80">&nbsp;</th>
								</tr>
							</thead>
							...
							<tbody>
								<c:forEach items="${page.result}" var="item">
									<tr>
										...
										<td>${item.fname}</td>
										<td>${ryMap[item.fbszzren].fname}</td>
										<td>${ryMap[item.fpsren].fname}</td>
										<%-- <td>${item.fscale}</td> --%>
										<td>${ryMap[item.ffuzeren].fname}</td>
										<td>${item.fzjdc}</td>
										<td>${item.ftbtjf}</td>
										<td>${item.ftype}</td>
										<td><a href="jyXm-info-input.do?xmid=${item.fid}&state=toubiao"
