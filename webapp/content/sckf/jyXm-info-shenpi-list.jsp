		...
		<!-- start of main -->
			<article class="m-widget">
				...
				<div id="jyXm-infoSearch" class="content content-inner">
					...
					<form name="jyXm-infoForm" method="post" action="jyXm-info-shenpi-list.do"
						class="form-inline">
						...
						<table id="jyXm-infoGrid" class="m-table table-hover table-bordered">
							<thead>
								<tr>
									...
									<th class="sorting" name="fshenpiren">审批人</th>
									<th class="sorting" name="fshenpiresult">审批结果</th>
									<th class="sorting" name="fshenpidate">审批时间</th>
									<th width="80">&nbsp;</th>
								</tr>
							</thead>
							<tbody>
								<c:forEach items="${page.result}" var="item">
									<tr>
										...
										<td>${item.ftze}</td>
										<td>${item.fkbdate}</td>
										<td>${ryMap[item.fshenpiren].fname}</td>
										<td>${item.fshenpiresult}</td>
										<td>${item.fshenpidate}</td>
										<td><a href="jyXm-info-shenpi.do?id=${item.fid}"
