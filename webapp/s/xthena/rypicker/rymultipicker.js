...
'<div id="' + conf.modalId + '" class="modal hide fade " style="width:800px;">'
			...
			success: function(data) {
				...
				for (var i = 0; i < data.result.length;i++) {
					...
					html +=
						...
						+'<td><a href="javascript:void(0);" onclick="chooseRy('+item.fid +','+i+')">选择 </label></td>'
					  +'</tr>';
					
					$('#ryPickerGrid').data('selectedItem' + i,item);
				}
...
function chooseRy(ryid,index){
	//alert(ryid);
	$('#selectedItem'+index).remove();
	var selected=$('#ryPickerGrid').data('selectedItem' + index);
	$('#picked').append('<tr id=gIndex'+gIndex+'><td><label>' + selected.fname + '</label></td>'
			+'<td><a href="javascript:void(0);" onclick="chooseCancel('+gIndex+')">取消</label></td></tr>');
	gIndex++;
}


function chooseCancel(index){
	$('#gIndex'+index).remove();
}
